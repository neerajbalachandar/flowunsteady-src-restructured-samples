...
function generate_monitor_dragonfly(vehicle, rho, Vinf, nsteps, save_path;
    ...
    if add_wings
        ...
        calc_aerodynamicforce_fun = generate_calc_aerodynamicforce(;
            add_parasiticdrag=false,
            add_skinfriction=false,
            airfoilpolar="xf-n0012-il-500000-n5.csv",
            parasiticdrag_args=()
        )

        fore_left_system = vehicle.tilting_systems[1]
        monitor_fore_left = uns.generate_monitor_wing(
            fore_left_system, Vinf, b_ref, ar_ref, rho, qinf, nsteps;
            calc_aerodynamicforce_fun=calc_aerodynamicforce_fun,
            save_path=save_path,
            run_name="fore_left_wing",
            CL_lbl="Fore Wing Left Lift (N)",
            CD_lbl="Fore Wing Left Drag (N)",
            L_dir=L_dir,
            D_dir=D_dir,
            wingmonitor_optargs...
        )

        fore_right_system = vehicle.tilting_systems[2]
        monitor_fore_right = uns.generate_monitor_wing(
            fore_right_system, Vinf, b_ref, ar_ref, rho, qinf, nsteps;
            calc_aerodynamicforce_fun=calc_aerodynamicforce_fun,
            save_path=save_path,
            run_name="fore_right_wing",
            CL_lbl="Fore Wing Right Lift (N)",
            CD_lbl="Fore Wing Right Drag (N)",
            L_dir=L_dir,
            D_dir=D_dir,
            wingmonitor_optargs...
        )

        hind_left_system = vehicle.tilting_systems[3]
        monitor_hind_left = uns.generate_monitor_wing(
            hind_left_system, Vinf, b_ref, ar_ref, rho, qinf, nsteps;
            calc_aerodynamicforce_fun=calc_aerodynamicforce_fun,
            save_path=save_path,
            run_name="hind_left_wing",
            CL_lbl="Hind Wing Left Lift (N)",
            CD_lbl="Hind Wing Left Drag (N)",
            L_dir=L_dir,
            D_dir=D_dir,
            wingmonitor_optargs...
        )

        hind_right_system = vehicle.tilting_systems[4]
        monitor_hind_right = uns.generate_monitor_wing(
            hind_right_system, Vinf, b_ref, ar_ref, rho, qinf, nsteps;
            calc_aerodynamicforce_fun=calc_aerodynamicforce_fun,
            save_path=save_path,
            run_name="hind_right_wing",
            CL_lbl="Hind Wing Right Lift (N)",
            CD_lbl="Hind Wing Right Drag (N)",
            L_dir=L_dir,
            D_dir=D_dir,
            wingmonitor_optargs...
        )
        ...
        push!(monitors, monitor_fore_left, monitor_fore_right, monitor_hind_left, monitor_hind_right)
    end
